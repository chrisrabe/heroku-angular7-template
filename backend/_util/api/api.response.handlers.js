...
const registerResponseHandlers = (express) => {
    ...
    /**
     * Sends client error > Forbidden with predefined messages for not having access to the requested store
     */
    express.response.cannotAccessStore = function () {
        setNoCacheHeaders(this);
        this.header('Content-Type', 'application/json; charset=utf-8');
        this.status(403);
        this.json({
            errorCode: errorCodes.forbidden.cannotAccessStore,
            errorMessage: 'You do not have access to the requested store',
        });
    };
